...
Class Action {
	...
	function save_venue(){
		...
		$data = " venue = '$venue' ";
		$data .= ", address = '$address' ";
        $description = addslashes($description); // Escape special characters
    ...
    function save_register(){
        ...
        // Secure input data
        $event_id = $this->db->real_escape_string($event_id);
        $name = $this->db->real_escape_string($name);
        $address = $this->db->real_escape_string($address);
        $email = $this->db->real_escape_string($email);
        $contact = $this->db->real_escape_string($contact);

        // Get current audience count for the event
        $check_capacity = $this->db->query("SELECT COUNT(*) as total FROM audience WHERE event_id = '$event_id'");
        if (!$check_capacity) {
            error_log("Error checking capacity: " . $this->db->error);
            return 0;
        }
        $audience_count = $check_capacity->fetch_assoc()['total'] ?? 0;

        // Get event audience capacity
        $check_event = $this->db->query("SELECT audience_capacity FROM events WHERE id = '$event_id'");
        if (!$check_event) {
            error_log("Error checking event: " . $this->db->error);
            return 0;
        }
        if ($check_event->num_rows > 0) {
            $event_capacity = $check_event->fetch_assoc()['audience_capacity'];

            // If audience count exceeds the capacity, return 2
            if ($audience_count >= $event_capacity) {
                return 2;
            }
        } else {
        ...
        if ($save) {
            ...
            error_log("Error inserting/updating audience: " . $this->db->error);
            return 0;
        }
    }
